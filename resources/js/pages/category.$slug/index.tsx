...
import { IVotingCategory } from '../votings/types/IVoting';
...
import { generateVotingUSSDCode } from '@/utils/common';
...
interface CategoryPageProps {
  slug: string;
  sessionId: string;
  categoryData: IVotingCategory | null;
  error: string | null;
}

export default function CategoryPage({ slug, sessionId, categoryData: initialCategoryData, error: initialError }: CategoryPageProps) {
  ...
  useEffect(() => {
    ...
    const updateCountdown = () => {
      setCountdown(formatCountdown(categoryData.end_time));
    };
  ...
  const handleBackToVoting = () => {
    if (categoryData) {
      router.visit(`/votings`);
    }
  };

  if (error) {
    return (
      <PublicLayout title="Category" subtitle="Error" description="Error loading category">
        <div className={styles.eventPage}>
          <div className={styles.errorContainer}>
            <div className={styles.errorIcon}>⚠️</div>
            <h2 className={styles.errorTitle}>Error Loading Category</h2>
  ...
  if (!categoryData) {
    return (
      <PublicLayout title="Category" subtitle="Not Found" description="Category not found">
        <div className={styles.eventPage}>
          <div className={styles.errorContainer}>
            <div className={styles.errorIcon}>📊</div>
            <h2 className={styles.errorTitle}>Category Not Found</h2>
            <p className={styles.errorMessage}>The requested category could not be found.</p>
            <button
              className={styles.backButton}
              onClick={() => router.visit('/votings')}
            >
  ...
  return (
    <PublicLayout
      ...
      description={`${categoryData.categories[0].name} category in ${categoryData.title}`}
      tags={[
        {
          label: `${isVotingEnded(categoryData.end_time) ? 'Ended' : `Ends In ${countdown}`}`,
        }
      ]}
      backgroundImage={getImageUrl(categoryData.cover_image)}
    >
      <div className={styles.eventPage}>
        {/* Category Header */}
          ...
          <div className={styles.container}>
            <div className={styles.categoryCard}>
              <div className={styles.categoryHeader}>
                <h1 className={styles.categoryTitle}>{categoryData.categories[0].name}</h1>
                <div className={styles.categoryMeta}>
                  <span className={styles.participantCount}>
                    {categoryData.categories[0].participants.length} participants
                  </span>
                  <span className={styles.winnersCount}>
                    {categoryData.categories[0].winners} winner{categoryData.categories[0].winners !== 1 ? 's' : ''}
                  </span>
                  <button
              ...
              <div className={styles.participantsGrid}>
                {categoryData.categories[0].participants.map((participant) => (
                  ...
                  //       <div className={styles.ussdCodeOverlay}>
                  //         USSD: {generateVotingUSSDCode(sessionId, participant.item_index.toString())}
                  //       </div>
                  //     </div>
                  //   </div>

                  //   <div className={styles.participantContent}>
                  //     <h4 className={styles.participantName}>{participant.name.toLowerCase() || 'Unknown Participant'}</h4>
                  //     <div className={styles.voteCount}>
                  //       <span className={styles.voteNumber}>{formatVotes(participant.votes || '0')}</span>
                  //       <span className={styles.voteLabel}>VOTES</span>
                  //     </div>

                  //     {!isVotingEnded(categoryData.end_time) && (
                  //       <Button
                  //         variant="primary"
                  ...
                  <ParticipantCard
                    ...
                    votingData={{
                      end_time: categoryData.end_time,
                    }}
                    setParticipantNameToVote={setParticipantNameToVote}
                    fetchPricingData={fetchPricingData}
                    shareUrl={`/vote/${sessionId}/${categoryData.categories[0].id}/${participant.slug}`}
                    shareTitle={`Share ${participant.name}`}
                    shareDescription={`Check out ${participant.name} in the ${categoryData.categories[0].name} category!`}
                  />
                ))}
              </div>
            </div>
